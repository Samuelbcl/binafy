...
// Usage : node scripts/rogner-capture.mjs captures/x.png captures/x-haut.png 2600 [depuis]
import { chromium } from 'playwright';
import { resolve } from 'node:path';
import { readFileSync, statSync } from 'node:fs';

// Quatrieme argument : la hauteur a partir de laquelle rogner (0 par defaut).
const [source, sortie, hauteur = '2600', depuis = '0'] = process.argv.slice(2);
const nav = await chromium.launch();
const page = await nav.newPage({ viewport: { width: 780, height: Number(hauteur) + Number(depuis) }, deviceScaleFactor: 0.6 });
...
await page.screenshot({ path: sortie, clip: { x: 0, y: Number(depuis), width: 780, height: Number(hauteur) } });
await nav.close();
